// Note:  disabling *any* checks for this because of CSV editing
 ...
 */
export function fromCsv(csvString: string): Csv {
    const regex = /,(?=(?:[^"]*"[^"]*")*(?![^"]*"))/;
...
export function toCsvString(data: Csv) {
    return [
 ...
 */
export function csvToObject(data: Csv): object[] {
    const checkForJson = (value: string): object | object[] | string => {
        if (!value) return ''
        value = unescapeCsv(value.trim())
        try {
            if (value.length >= 2) {
                const first = value[0]
 ...
 */
export function csvFromObject(data: unknown): Csv {
