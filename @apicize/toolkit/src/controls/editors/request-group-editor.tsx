...
import { useWorkspace, GroupPanel, WorkspaceStore } from '../../contexts/workspace.context';
...
interface GroupPanelViewProps {
    ...
    usePanel: GroupPanel
    hasWarnings: boolean
    onPanelChanged: (event: React.SyntheticEvent, newValue: GroupPanel) => void
}
...
interface GroupEditorLayoutProps {
    group: EditableRequestGroup
    settings: EditableSettings
...
const GroupEditorLayout = observer(({ group, settings, workspace, usePanel, hasWarnings, onPanelChanged, sx }: GroupEditorLayoutProps) => {
    ...
    return group.resultMenuItems.length > 0 && group.selectedResultMenuItem
        ...
        : <Box className='editor group single-panel' sx={sx}>
            <GroupPanelView group={group} settings={settings} usePanel={usePanel} hasWarnings={hasWarnings} onPanelChanged={onPanelChanged} />
        </Box>
})
...
export const RequestGroupEditor = observer(({ group, sx }: { group: EditableRequestGroup, sx?: SxProps }) => {
    ...
    const hasWarnings = group.validationWarnings.hasEntries

    if (!hasWarnings && usePanel === 'Warnings') {
        usePanel = 'Info'
    }

    return <GroupEditorLayout
        group={group}
        settings={settings}
        workspace={workspace}
        usePanel={usePanel}
        hasWarnings={hasWarnings}
        onPanelChanged={handlePanelChanged}
        sx={sx}
    />
})