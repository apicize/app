import { BodyConversion } from '../../src/services/body-conversion';
import { Body, BodyType } from '@apicize/lib-typescript';

describe('BodyConversion', () => {
    describe('conversions', () => {
        const testBase64 = ""
        const testJson = JSON.stringify({ quote: "{{test-quote}}" })
        const testXml = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><root><author>{{author}}</author><quote>{{quote}}</quote></root>`
        const testNameValuePairs = [{ name: 'foo1', value: 'bar1' }, { name: 'foo2', value: 'bar2' }]

        const stripWhite = (s: string) => s.replaceAll(/\s/g, '')

        it('are consistent from binary, text and back', async () => {
            const asText = await (new BodyConversion({
                type: BodyType.Raw,
                data: testBase64
            })).toText()
            const asBinary = await (new BodyConversion(asText)).toRaw()
            expect(asBinary.data).toEqual(testBase64)
        })

        it('are consistent from JSON, text and back', async () => {
            const asText = await (new BodyConversion({
                type: BodyType.JSON,
                data: testJson
            })).toText()
            const asJson = await (new BodyConversion(asText)).toJson()
            expect(stripWhite(asJson.data)).toEqual(stripWhite(testJson))
        })

        it('are consistent from JSON, XML and back', async () => {
            const asXML = await (new BodyConversion({
                type: BodyType.JSON,
                data: testJson
            })).toXML()
            const asJson = await (new BodyConversion(asXML)).toJson()
            expect(stripWhite(asJson.data)).toEqual(stripWhite(testJson))
        })

        it('are consistent from XML, text and back', async () => {
            const asText = await (new BodyConversion({
                type: BodyType.XML,
                data: testXml
            })).toText()
            const asXml = await (new BodyConversion(asText)).toXML()
            expect(stripWhite(asXml.data)).toEqual(stripWhite(testXml))
        })

        it('are consistent from JSON, XML, text and back', async () => {
            const asXml = await (new BodyConversion({
                type: BodyType.JSON,
                data: testJson
            })).toXML()
            const asText = await (new BodyConversion(asXml)).toText()
            const asJson = await (new BodyConversion(asText)).toJson()

            expect(stripWhite(asJson.data)).toEqual(stripWhite(testJson))
        })

        it('are consistent between form, text and back', async () => {
            const asText = await (new BodyConversion({
                type: BodyType.Form,
                data: testNameValuePairs
            })).toText()
            const asForm = await (new BodyConversion(asText)).toForm()
            // Compare without IDs
            expect(asForm.data.map(d => ({ name: d.name, value: d.value }))).toEqual(testNameValuePairs)
        })
    })

    describe('parseText', () => {
        it('should return undefined for empty string', async () => {
            const result = await BodyConversion['parseText']('');
            expect(result).toBeUndefined();
        });

        it('should return undefined for null', async () => {
            const result = await BodyConversion['parseText'](null as any);
            expect(result).toBeUndefined();
        });

        it('should return undefined for undefined', async () => {
            const result = await BodyConversion['parseText'](undefined as any);
            expect(result).toBeUndefined();
        });

        it('should parse valid JSON', async () => {
            const json = '{"name": "John", "age": 30}';
            const result = await BodyConversion['parseText'](json);
            expect(result).toEqual({ name: 'John', age: 30 });
        });

        it('should parse valid JSON array', async () => {
            const json = '[1, 2, 3]';
            const result = await BodyConversion['parseText'](json);
            expect(result).toEqual([1, 2, 3]);
        });

        it('should parse valid XML', async () => {
            const xml = '<root><name>John</name></root>';
            const result = await BodyConversion['parseText'](xml);
            expect(result).toHaveProperty('name');
        });

        it('should parse name-value pairs separated by commas', async () => {
            const pairs = 'key1=value1,key2=value2,key3=value3';
            const result = await BodyConversion['parseText'](pairs);
            expect(result).toEqual([
                { name: 'key1', value: 'value1' },
                { name: 'key2', value: 'value2' },
                { name: 'key3', value: 'value3' }
            ])
        });

        it('should handle escaped commas in name-value pairs', async () => {
            const pairs = 'key1=value1\\,a,key2=value2';
            const result = await BodyConversion['parseText'](pairs);
            expect(result).toEqual([
                { name: 'key1', value: 'value1,a' },
                { name: 'key2', value: 'value2' },
            ]);
        });

        it('should parse plain text as a string', async () => {
            const text = 'plain text that is not JSON or XML';
            const result = await BodyConversion['parseText'](text);
            expect(result).toEqual(text);
        });

        it('should parse text with commas without equals as a string', async () => {
            const text = 'plain text, with commas, gets parsed';
            const result = await BodyConversion['parseText'](text);
            expect(result).toEqual(text);
        });

        it('should parse text with commas with equals as name value pairs', async () => {
            const text = 'abc=123, def=456';
            const result = await BodyConversion['parseText'](text);
            expect(result).toEqual([
                { name: 'abc', value: '123' },
                { name: 'def', value: '456' },
            ]);
        });

        it('should parse text with commas with escaped equals as name value pairs', async () => {
            const text = 'abc\\==123, def=456\\=';
            const result = await BodyConversion['parseText'](text);
            expect(result).toEqual([
                { name: 'abc=', value: '123' },
                { name: 'def', value: '456=' },
            ])
        });

        it('should parse text with commas with escaped commas as name value pairs', async () => {
            const text = 'abc\\,=123, def=456\\,';
            const result = await BodyConversion['parseText'](text);
            expect(result).toEqual([
                { name: 'abc,', value: '123' },
                { name: 'def', value: '456,' },
            ])
        });

        it('should return Base64 when there is valid Base64 when checkBase64 is true', async () => {
            const base64Text = 'SGVsbG8gV29ybGQ=';
            const result = await BodyConversion['parseText'](base64Text, true);
            expect(result).toEqual(base64Text);
        });

        it('should decode Base64 JSON and parse it', async () => {
            const base64Json = 'eyJuYW1lIjogIkpvaG4iLCAiYWdlIjogMzB9';
            const result = await BodyConversion['parseText'](base64Json, true);
            expect(result).toEqual({ name: 'John', age: 30 });
        });

        it('should decode Base64 XML and parse it', async () => {
            const base64Xml = 'PHJvb3Q+PG5hbWU+VGVzdDwvbmFtZT48L3Jvb3Q+';
            const result = await BodyConversion['parseText'](base64Xml, true);
            expect(result).toHaveProperty('name');
        });

        it('should use checkBase64=true by default', async () => {
            const base64Json = 'eyJ0ZXN0IjogInZhbHVlIn0=';
            const result = await BodyConversion['parseText'](base64Json);
            expect(result).toEqual({ test: 'value' });
        });
    });

    describe('isValidBase64', () => {
        it('should return true for valid Base64 strings', () => {
            expect(BodyConversion['isValidBase64']('SGVsbG8gV29ybGQ=')).toBe(true);
            expect(BodyConversion['isValidBase64']('VGVzdA==')).toBe(true);
            expect(BodyConversion['isValidBase64']('YWJjZGVmZ2hpamtsbW5vcHFyc3R1dnd4eXo=')).toBe(true);
        });

        it('should return false for invalid Base64 strings', () => {
            expect(BodyConversion['isValidBase64']('Invalid@Base64')).toBe(false);
            expect(BodyConversion['isValidBase64']('Hello World')).toBe(false);
            expect(BodyConversion['isValidBase64']('123')).toBe(false);
        });

        it('should return false for empty or null strings', () => {
            expect(BodyConversion['isValidBase64']('')).toBe(false);
            expect(BodyConversion['isValidBase64'](null as any)).toBe(false);
            expect(BodyConversion['isValidBase64'](undefined as any)).toBe(false);
        });

        it('should return false for strings with incorrect length', () => {
            expect(BodyConversion['isValidBase64']('SGVsb')).toBe(false);
        });

        it('should return false for strings with too much padding', () => {
            expect(BodyConversion['isValidBase64']('SGVsbG8===')).toBe(false);
        });

        it('should return true for image data', () => {
            const testBase64 = ""
            expect(BodyConversion['isValidBase64'](testBase64)).toBe(true);
        })
    });

    describe('toXML', () => {
        it('should return source when body type is already XML', async () => {
            const xmlBody: Body = {
                type: BodyType.XML,
                data: '<root><item>value</item></root>'
            };
            const converter = new BodyConversion(xmlBody);
            const result = await converter.toXML();

            expect(result).toEqual(xmlBody);
        });

        it('should convert JSON body to XML', async () => {
            const jsonBody: Body = {
                type: BodyType.JSON,
                data: '{"root": {"item": "value"}}'
            };
            const converter = new BodyConversion(jsonBody);
            const result = await converter.toXML();

            expect(result.type).toBe(BodyType.XML);
            expect(result.data).toContain('<root>');
            expect(result.data).toContain('<item>value</item>');
        });

        it('should convert Text body to XML', async () => {
            const textBody: Body = {
                type: BodyType.Text,
                data: '{"root": {"item": "value"}}'
            };
            const converter = new BodyConversion(textBody);
            const result = await converter.toXML();

            expect(result.type).toBe(BodyType.XML);
            expect(typeof result.data).toBe('string');
        });

        it('should convert Form body to XML', async () => {
            const formBody: Body = {
                type: BodyType.Form,
                data: [
                    { name: 'key1', value: 'value1' },
                    { name: 'key2', value: 'value2' }
                ]
            };
            const converter = new BodyConversion(formBody);
            const result = await converter.toXML();

            expect(result.type).toBe(BodyType.XML);
            expect(typeof result.data).toBe('string');
        });

        it('should convert Raw body to XML', async () => {
            const rawBody: Body = {
                type: BodyType.Raw,
                data: 'raw data content'
            };
            const converter = new BodyConversion(rawBody);
            const result = await converter.toXML();

            expect(result.type).toBe(BodyType.XML);
            expect(typeof result.data).toBe('string');
        });

        it('should return empty string for None body type', async () => {
            const noneBody: Body = {
                type: BodyType.None,
                data: undefined
            };
            const converter = new BodyConversion(noneBody);
            const result = await converter.toXML();

            expect(result.type).toBe(BodyType.XML);
            expect(result.data).toBe('');
        });

        it('should handle complex nested JSON to XML conversion', async () => {
            const jsonBody: Body = {
                type: BodyType.JSON,
                data: '{"root": {"child1": {"name": "value1"}, "child2": {"name": "value2"}}}'
            };
            const converter = new BodyConversion(jsonBody);
            const result = await converter.toXML();

            expect(result.type).toBe(BodyType.XML);
            expect(result.data).toContain('<root>');
            expect(result.data).toContain('<child1>');
            expect(result.data).toContain('<child2>');
        });
    });

    describe('toText', () => {
        it('should return source when body type is already Text', async () => {
            const textBody: Body = {
                type: BodyType.Text,
                data: 'plain text content'
            };
            const converter = new BodyConversion(textBody);
            const result = await converter.toText();

            expect(result).toEqual(textBody);
        });

        it('should convert JSON body to Text', async () => {
            const jsonBody: Body = {
                type: BodyType.JSON,
                data: '{"name": "John", "age": 30}'
            };
            const converter = new BodyConversion(jsonBody);
            const result = await converter.toText();

            expect(result.type).toBe(BodyType.Text);
            expect(result.data).toBe('{"name": "John", "age": 30}');
        });

        it('should convert XML body to Text', async () => {
            const xmlBody: Body = {
                type: BodyType.XML,
                data: '<root><item>value</item></root>'
            };
            const converter = new BodyConversion(xmlBody);
            const result = await converter.toText();

            expect(result.type).toBe(BodyType.Text);
            expect(result.data).toBe('<root><item>value</item></root>');
        });

        it('should convert Raw body to Text', async () => {
            const rawBody: Body = {
                type: BodyType.Raw,
                data: 'raw data content'
            };
            const converter = new BodyConversion(rawBody);
            const result = await converter.toText();

            expect(result.type).toBe(BodyType.Text);
            expect(result.data).toBe('raw data content');
        });

        it('should convert Form body to Text with comma-separated name-value pairs', async () => {
            const formBody: Body = {
                type: BodyType.Form,
                data: [
                    { name: 'key1', value: 'value1' },
                    { name: 'key2', value: 'value2' },
                    { name: 'key3', value: 'value3' }
                ]
            };
            const converter = new BodyConversion(formBody);
            const result = await converter.toText();

            expect(result.type).toBe(BodyType.Text);
            expect(result.data).toBe('key1=value1, key2=value2, key3=value3');
        });

        it('should escape first comma in Form field names and values', async () => {
            const formBody: Body = {
                type: BodyType.Form,
                data: [
                    { name: 'key,with,comma', value: 'value,with,comma' },
                    { name: 'key2', value: 'value2' }
                ]
            };
            const converter = new BodyConversion(formBody);
            const result = await converter.toText();

            expect(result.type).toBe(BodyType.Text);
            // Note: replace() only replaces first occurrence, so only first comma is escaped
            expect(result.data).toContain('key\\,with,comma=value\\,with,comma');
            expect(result.data).toContain('key2=value2');
        });

        it('should return empty string for None body type', async () => {
            const noneBody: Body = {
                type: BodyType.None,
                data: undefined
            };
            const converter = new BodyConversion(noneBody);
            const result = await converter.toText();

            expect(result.type).toBe(BodyType.Text);
            expect(result.data).toBe('');
        });

        it('should handle Form body with single name-value pair', async () => {
            const formBody: Body = {
                type: BodyType.Form,
                data: [
                    { name: 'singleKey', value: 'singleValue' }
                ]
            };
            const converter = new BodyConversion(formBody);
            const result = await converter.toText();

            expect(result.type).toBe(BodyType.Text);
            expect(result.data).toBe('singleKey=singleValue');
        });

        it('should handle empty Form body', async () => {
            const formBody: Body = {
                type: BodyType.Form,
                data: []
            };
            const converter = new BodyConversion(formBody);
            const result = await converter.toText();

            expect(result.type).toBe(BodyType.Text);
            expect(result.data).toBe('');
        });
    });

    describe('toRaw', () => {
        it('should return source when body type is already Raw', async () => {
            const rawBody: Body = {
                type: BodyType.Raw,
                data: 'SGVsbG8gV29ybGQ='
            };
            const converter = new BodyConversion(rawBody);
            const result = await converter.toRaw();

            expect(result).toEqual(rawBody);
        });

        it('should convert Text body to Base64-encoded Raw', async () => {
            const textBody: Body = {
                type: BodyType.Text,
                data: 'Hello World'
            };
            const converter = new BodyConversion(textBody);
            const result = await converter.toRaw();

            expect(result.type).toBe(BodyType.Raw);
            expect(result.data).toBe('SGVsbG8gV29ybGQ=');
        });

        it('should convert JSON body to Base64-encoded Raw', async () => {
            const jsonBody: Body = {
                type: BodyType.JSON,
                data: '{"name":"test"}'
            };
            const converter = new BodyConversion(jsonBody);
            const result = await converter.toRaw();

            expect(result.type).toBe(BodyType.Raw);
            expect(result.data).toBeTruthy();
            expect(result.data.length % 4).toBe(0);
            const decoded = atob(result.data);
            expect(decoded).toBe('{"name":"test"}');
        });

        it('should convert XML body to Base64-encoded Raw', async () => {
            const xmlBody: Body = {
                type: BodyType.XML,
                data: '<root><item>value</item></root>'
            };
            const converter = new BodyConversion(xmlBody);
            const result = await converter.toRaw();

            expect(result.type).toBe(BodyType.Raw);
            expect(result.data).toBeTruthy();
            // Decode and verify original content
            const decoded = atob(result.data);
            expect(decoded).toBe('<root><item>value</item></root>');
        });

        it('should convert Form body to Base64-encoded JSON Raw', async () => {
            const formBody: Body = {
                type: BodyType.Form,
                data: [
                    { name: 'key1', value: 'value1' },
                    { name: 'key2', value: 'value2' }
                ]
            };
            const converter = new BodyConversion(formBody);
            const result = await converter.toRaw();

            expect(result.type).toBe(BodyType.Raw);
            expect(result.data).toBeTruthy();
            // Decode and verify it's JSON array
            const decoded = atob(result.data);
            const parsed = JSON.parse(decoded);
            expect(Array.isArray(parsed)).toBe(true);
            expect(parsed).toHaveLength(2);
            expect(parsed[0]).toHaveProperty('name', 'key1');
            expect(parsed[0]).toHaveProperty('value', 'value1');
        });

        it('should return empty string for None body type', async () => {
            const noneBody: Body = {
                type: BodyType.None,
                data: undefined
            };
            const converter = new BodyConversion(noneBody);
            const result = await converter.toRaw();

            expect(result.type).toBe(BodyType.Raw);
            expect(result.data).toBe('');
        });

        it('should handle UTF-8 characters correctly', async () => {
            const textBody: Body = {
                type: BodyType.Text,
                data: 'Hello 世界 🌍'
            };
            const converter = new BodyConversion(textBody);
            const result = await converter.toRaw();

            expect(result.type).toBe(BodyType.Raw);
            expect(result.data).toBeTruthy();
            // Decode and verify UTF-8 is preserved
            const decoder = new TextDecoder();
            const decoded = atob(result.data);
            const bytes = new Uint8Array(decoded.length);
            for (let i = 0; i < decoded.length; i++) {
                bytes[i] = decoded.charCodeAt(i);
            }
            expect(decoder.decode(bytes)).toBe('Hello 世界 🌍');
        });

        it('should handle empty Text body', async () => {
            const textBody: Body = {
                type: BodyType.Text,
                data: ''
            };
            const converter = new BodyConversion(textBody);
            const result = await converter.toRaw();

            expect(result.type).toBe(BodyType.Raw);
            expect(result.data).toBe('');
        });

        it('should handle empty Form body', async () => {
            const formBody: Body = {
                type: BodyType.Form,
                data: []
            };
            const converter = new BodyConversion(formBody);
            const result = await converter.toRaw();

            expect(result.type).toBe(BodyType.Raw);
            // Empty array encoded to JSON then Base64
            const decoded = atob(result.data);
            expect(JSON.parse(decoded)).toEqual([]);
        });
    });
});

