...
A Scenario is a set of variables that are available when testing [requests](help:workspace/requests).  These can be set for any [Request](help:workspace/requests) or [Group](help:workspace/groups).  If they are set for a Group, they will apply to call child Requests or Groups.  At the simplest level, you can use this capability to inject different values into the same Request(s), without having to modify them.
